#!/usr/bin/env python3
"""
Investment Desk - Workflow Orchestration con Fail-Fast (SSOT)
...
Uso:
    python run_investment_desk.py --hours-back 48
"""
...
def run_investment_desk(hours_back: int = 48) -> Dict:
    ...
    logger.info("="*70)
    logger.info("🏛️  INVESTMENT DESK - FAIL-FAST WORKFLOW")
    logger.info("="*70)
    ...
    try:
        ...
        def load_watchlist():
            try:
                ...
                # Buscar la watchlist por ID o usar la primera si no se encuentra
                watchlist = next((wl for wl in watchlists if wl.get('id') == default_id), None)
                
                # Fallback a la primera si no existe la de por defecto
                if not watchlist and watchlists:
                    watchlist = watchlists[0]
                    logger.warning(f"   ⚠️  Default watchlist ID {default_id} not found. Using first available: {watchlist.get('name')}")
        ...
        for i, ticker in enumerate(all_tickers, 1):
            ...
            try:
                # Extraer posición del portafolio si existe para habilitar Check Mecánico
                position = None
                if is_portfolio:
                    position = next((p for p in portfolio_result.get('positions', []) if p['symbol'] == ticker), None)

                result = analyze_ticker_module.analyze_ticker(ticker, hours_back=hours_back, portfolio_position=position)
        ...
        def aggregate_results():
            try:
                ...
                cautious = [t for t in ticker_results if t['recommendation'] in ['CAUTIOUS', 'HIGH_VOLATILITY']]
                neutral = [t for t in ticker_results if t['recommendation'] in ['NEUTRAL', 'DATA_ERROR']]
                ...
                # Metrics
                data_errors = [t for t in ticker_results if t['recommendation'] == 'DATA_ERROR']
                if data_errors:
                    logger.warning(f"⚠️  {len(data_errors)} tickers had DATA ERRORS and were excluded from action.")
                ...
                total_insights = sum(t['unique_entities_found'] for t in ticker_results)
                ...
                return {
                    ...
                    'total_insights_found': total_insights,
        ...
        def persist_results():
            try:
                ...
                # Calcular agregados para persistencia
                total_news = sum(tr.get('related_news_count', 0) for tr in ticker_results)
                total_entities = sum(len(tr.get('mapped_entities', [])) for tr in ticker_results)
                
                avg_conf = sum(tr.get('avg_confidence', 0) for tr in ticker_results) / len(ticker_results) if ticker_results else 0
                avg_pos = sum(tr.get('positive_ratio', 0) for tr in ticker_results) / len(ticker_results) if ticker_results else 0
                avg_neg = sum(tr.get('negative_ratio', 0) for tr in ticker_results) / len(ticker_results) if ticker_results else 0
                ...
                desk_analysis = {
                    ...
                    'avg_positive_ratio': avg_pos,
                    'avg_negative_ratio': avg_neg,
                    'ticker_results': ticker_results,
                    **aggregation_result,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Run Investment Desk')
    parser.add_argument('--hours-back', type=int, default=48,
                       help='Hours to look back (default: 48)')

    args = parser.parse_args()

    result = run_investment_desk(hours_back=args.hours_back)
