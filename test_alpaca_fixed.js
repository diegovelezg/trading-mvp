...
async function getTickerData(ticker) {
  try {
    ...
    // 2. Get historical bars with start date
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - 35);
    const startDateStr = startDate.toISOString();

    const barsResponse = await fetchFromAlpaca(
      `/stocks/${ticker}/bars?timeframe=1Day&start=${startDateStr}`
    );
    ...
    if (bars.length >= 28) {
      ...
      console.log(`     Price 28d ago: $${price28DaysAgo}`);
...
async function main() {
  console.log('🚀 Testing CORRECTED Alpaca Data API Integration\n');

  const tickers = ['CCJ', 'NXE', 'UEC'];
