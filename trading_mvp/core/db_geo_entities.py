    ...
    with conn.cursor() as cur:
        cur.execute("""
            CREATE TABLE IF NOT EXISTS geo_macro_entities (
                ...
                sectors JSONB,                -- ["Energy", "Technology"]
                regions JSONB,                -- ["Middle East", "US"]
...
def insert_entity(news_id: int, entity: Dict, model_used: str = None) -> Optional[int]:
    ...
    try:
        with conn.cursor() as cur:
            ...
            sectors_json = json.dumps(entity.get('sectors', []))
            regions_json = json.dumps(entity.get('regions', []))

            cur.execute("""
                INSERT INTO geo_macro_entities
                (news_id, entity_name, entity_type, impact, confidence, sectors, regions, model_used)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                ON CONFLICT (news_id, entity_name) DO NOTHING
                RETURNING id
            """, (
                news_id,
            ...
            if result:
                ...
                cur.execute("""
                    ...
                    WHERE news_id = %s AND entity_name = %s
...
def get_recent_entities(hours_back: int = 24, limit: int = 100) -> List[Dict]:
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute("""
                SELECT * FROM geo_macro_entities
                WHERE extracted_at > CURRENT_TIMESTAMP - (INTERVAL '1 hour' * %s)
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute("""
                SELECT * FROM geo_macro_entities
                WHERE entity_name = %s
                AND extracted_at > CURRENT_TIMESTAMP - (INTERVAL '1 hour' * %s)
                ORDER BY extracted_at DESC
